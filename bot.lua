...
function get_bot (i, amwr)
	function bot_info (i, amwr)
		redis:set("botBOT-IDid",amwr.id_)
		if naji.first_name_ then
			redis:set("botBOT-IDfname",amwr.first_name_)
		end
		if naji.last_name_ then
			redis:set("botBOT-IDlanme",amwr.last_name_)
		end
		redis:set("botBOT-IDnum",amwr.phone_number_)
		return naji.id_
	end
...
function process_join(i, amwr)
	if naji.code_ == 429 then
		local message = tostring(amwr.message_)
		...
		if msg.content_.ID == "MessageText" then
			...
			if is_naji(msg) then
				...
				elseif text:match("^(تازه سازی)$")then
					...
					return send(msg.chat_id_,msg.id_,"<i>تازه‌سازی آمار پاور شماره </i><code> BOT-ID </code> با موفقیت انجام شد.")
				elseif text:match("^(وضعیت)$") then
					...
					local txtadd = redis:get("botBOT-IDaddmsgtext") or  "ادد شدی بیا پی وی "
